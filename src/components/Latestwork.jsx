...
import "./Latestwork.css"; // Optional for any hover logic
...
const projects = [
  ...
  },
  {
    ...
    description:
      "A responsive school or college website with admin, faculty and student panel.",
    video: video2,
  ...
  },
  {
    ...
    description:
      "Personal portfolio with neumorphic design and animations.",
    video: video3,
    tech: ["html5", "css3", "react"],
...
const Latestwork = () => {
    ...
    responsive: [
      {
        breakpoint: 768,
        settings: {
          centerMode: false, // ✅ disable on mobile
          centerPadding: "0px",
          slidesToShow: 1,
        },
      },
  ...
  return (
    <div className="w-full overflow-x-hidden bg-black text-gray-800 px-4 sm:px-10 py-16">
      <h1 className="text-md sm:text-xl font-semibold text-center text-white mb-16">
...
const Card = ({ title, description, video, tech, link, status }) => {
  return (
    <div className="bg-gray-800 overflow-hidden h-full border-r-8 border-slate-900  rounded-md">
      ...
      <div className="p-4 text-center">
        ...
        <p className="text-xs sm:text-sm text-white mt-2">{description}</p>
        <div className="text-sm text-white mt-3 font-medium">
          ...
          {tech.includes("angular") && (
            <FontAwesomeIcon icon={faAngular} className="text-red-600 mx-1" />
          )}
        </div>
        <div className="flex items-center justify-between mt-4">
          <p className="px-4 py-2 text-xs sm:text-sm font-semibold bg-[#e0e0e0] rounded text-red-500">
            {status}
          </p>
          <a
            href={link}
            ...
            target="_blank"
            className="px-4 py-2 text-xs sm:text-sm font-semibold bg-[#e0e0e0] rounded text-green-600
            hover:shadow-[inset_4px_4px_8px_#bebebe,inset_-4px_-4px_8px_#ffffff]
            transition duration-200 flex items-center"
          >
            Live
            <FontAwesomeIcon icon={faLink} className="ml-2 text-green-600" />
          </a>
        </div>
      </div>
    </div>
